...
\begin{tikzpicture}[
    ...
    box/.style={rectangle, draw, minimum width=2.2cm, minimum height=0.8cm, text centered, rounded corners=2pt, fill=white, align=center},
    framebox/.style={box, draw=blue!60, fill=blue!10},
    orchbox/.style={box, draw=green!60, fill=green!10, minimum width=8cm},
    mcpbox/.style={box, draw=orange!60, fill=orange!10, minimum width=8cm},
    toolbox/.style={box, draw=purple!60, fill=purple!10, minimum width=1.8cm},
    arrow/.style={-{Stealth[length=2mm]}, thick}
]

% Orchestrator
\node[orchbox] (orch) at (0,0) {\textbf{Parallel Orchestrator}};

% Frameworks
\node[framebox] (f1) at (-3,-1.8) {Claude SDK};
\node[framebox] (f2) at (-1,-1.8) {Google ADK};
\node[framebox] (f3) at (1,-1.8) {AWS Strands};
\node[framebox] (f4) at (3,-1.8) {CrewAI};

% MCP Server
\node[mcpbox] (mcp) at (0,-3.6) {\textbf{MCP Server (FastMCP)}};

% Tools
\node[toolbox] (t1) at (-3,-5.4) {Customer\\API};
\node[toolbox] (t2) at (-1,-5.4) {Orders\\API};
\node[toolbox] (t3) at (1,-5.4) {Knowledge\\Base};
\node[toolbox] (t4) at (3,-5.4) {Refund\\Process};
...
% Labels
\node[anchor=east, font=\scriptsize] at (-4.5,0) {\textit{Orchestration}};
\node[anchor=east, font=\scriptsize] at (-4.5,-1.8) {\textit{Frameworks}};
\node[anchor=east, font=\scriptsize] at (-4.5,-3.6) {\textit{Tool Server}};
\node[anchor=east, font=\scriptsize] at (-4.5,-5.4) {\textit{Tools}};

\end{tikzpicture}
\caption{Arena system architecture. The orchestrator spawns all four framework processes in parallel to eliminate position bias. Each framework connects to an identical MCP server providing five customer support tools, ensuring fair comparison.}
...
\begin{axis}[
    ...
    ymin=50, ymax=95,
    legend style={at={(0.5,-0.35)}, anchor=north, legend columns=2, font=\footnotesize},
    ymajorgrids=true,
    grid style=dashed,
...
\end{axis}
\end{tikzpicture}
\caption{Framework correctness comparison across scenario types. Note the dramatic ranking reversal: Google ADK excels on simple scenarios (91.67\%) but degrades 36\% on complex scenarios (58.33\%), while Claude SDK maintains robustness across complexity levels.}
...
\begin{axis}[
    ...
    ylabel={Correctness (\%)},
    symbolic x coords={Simple, Complex},
    xtick=data,
...
\addplot[color=green!70!black, mark=diamond*, thick] coordinates {
    (Simple,82.67)
    (Complex,75.00)
};
...
\end{axis}
\end{tikzpicture}
\caption{Framework scalability analysis showing performance degradation from simple (T1-T3) to complex (T4) scenarios. Claude SDK and CrewAI maintain robustness (<15\% drop) while speed-optimized frameworks (Google ADK, AWS Strands) exhibit substantial degradation (>20\% drop).}
...
Effect size analysis (Cohen's d) for Claude SDK comparisons: vs. AWS Strands: d=$\infty$ (perfect separation), vs. Google ADK: d=3.52 (very large), vs. CrewAI: d=4.47 (very large). Power analysis confirms sample size (n=24) provides >0.99 power to detect these differences, validating statistical rigor.
