"""Run benchmark for AWS Strands, Google ADK, and CrewAI."""
import json
import time
...
def main():
    """Run benchmark for new frameworks only."""
    print("\n" + "="*70)
    print("ARENA: AWS Strands, Google ADK & CrewAI Benchmark")
    print("="*70)
    ...
    for fw_name, fw_results in all_results.items():
        if "error" in fw_results:
            print(f"\n{fw_name}: ERROR - {fw_results['error']}")
            continue
